...
/**
 * Manages the results from BUGnet searches keeps track of our current search
 * term, count, and page number Used by BugnetSearchSectionHelper
 * 
 * @author brian
 * 
 */
public class BugnetResultManager implements IPropertyChangeListener {
	...
	/**
	 * Making this a singleton allows us to keep track of current page # and
	 * count over different requests
	 * 
	 * @return
	 */
	...
	private String toQueryString() {
		String querystring = "";
		if (search != null)
			querystring = addValue(querystring, "search", search);
		if (count > 0)
			querystring = addValue(querystring, "count", count + "");
		if (page > 0)
			querystring = addValue(querystring, "current_page", page + "");
		if (category.startsWith(BugnetApplicationCategoryHelper.BUG_CONNECTION_CATEGORY_PREFIX) && packages != null) {
