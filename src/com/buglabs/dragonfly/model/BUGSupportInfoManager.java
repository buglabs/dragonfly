...
/**
 * This class helps manage the SDK's view of the BUG support info
 * The support information is used to determine the compatibility between
 * a BUG and the BUG Apps.  Currently, this class is used when attempting
 * to upload a BUG app to a BUG.
 * 
 * @author bballantine
 *
 */
public class BUGSupportInfoManager {
	 ...
	 * 	We mostly care if it's J2SE-1.1 (PhoneME) or Java2SE-1.6 (OpenJDK 6)
	 * 
	 * @return
	 */
	...
	/**
	 * Static method to create a new BUGSupportInfoManager from
	 *  a connected BUG
	 * 
	 * @param bug
	 * @return
	...
	public static BUGSupportInfoManager load(Bug bug) {
		...
		try {
			info = parseInfoXml(BugWSHelper.
					getBUGSupportInfo(bug.getSupportURL()));
		} catch (IOException e) {
			info = null;
		}
	...
	private static BUGSupportInfoManager parseInfoXml(XmlNode infoRoot) {
		...
		XmlNode node;
		for (Iterator<XmlNode> itr = 
				propertiesNode.getChildren().iterator(); itr.hasNext();) {
			node = (XmlNode) itr.next();
			if (node.getAttribute(JVM_PROPERTY_KEY) != null 
					&& node.getAttribute(JVM_PROPERTY_KEY).
					equals(JVM_EXECUTION_ENVIRONMENT_KEY)) {
		...
		for (Iterator<XmlNode> itr = 
				bundlesNode.getChildren().iterator(); itr.hasNext();) {
			node = itr.next();
	...
	/**
	 * Older Bugs don't have support xml, so we try to
	 * get the data a different way
	 * 
	 * @param bug
	 * @return
	 */
	...
	/**
	 * Helper to parse the rootfs version text
	 * which is /etc/buildinfo on the bug
	 * 
	 * @param versionText
	 * @return
	 */
