...
public class CodeGenerationPage extends WizardPage implements IDebugEventSetListener {
	...
	private void createServicesSection(final Composite mainComposite) {
		...
		txtFilter.addModifyListener(new ModifyListener() {
			public void modifyText(ModifyEvent e) {
				...
				serviceFilter.setPattern("(?i).*" + filter + ".*", Arrays.asList(dependencyViewer.getCheckedElements()));
		...
		dependencyViewer.addDoubleClickListener(new IDoubleClickListener() {
			public void doubleClick(DoubleClickEvent event) {
				...
				openServicePropertySelectorDialog(mainComposite.getShell(), selectedService, false);
			}
		});
		...
		refreshServiceDefintions.addSelectionListener(new SelectionListener() {
			...
			public void widgetSelected(SelectionEvent e) {
				...
				if (!reloadListDialog(mainComposite.getShell()))
					return;
		...
		serviceDescriptionArea.setForeground(compServices.getDisplay().getSystemColor(SWT.COLOR_DARK_GRAY));
	...
	private void openServicePropertySelectorDialog(Shell shell, String selectedService, boolean clearCheckedOnCancel) {
		...
		if (servicePropertyOptionsMap.containsKey(selectedService) && servicePropertyOptionsMap.get(selectedService) != null
			...
			Dialog d = new ServicePropertySelectorDialog(shell, selectedService, (List<ServicePropertyHelper>) servicePropertyOptionsMap.get(selectedService), pinfo);
	...
	private void populateServiceList(BugConnection bugConnection, IProgressMonitor monitor) {
		try {
			...
			if (details == null || details.size() < 1) {
				List<String> services = BugWSHelper.getAllServices(bugConnection.getProgramURL());
				details = new ArrayList<ServiceDetail>();
				for (String service : services) {
					details.add(new ServiceDetail(service, new ArrayList<ServiceProperty>()));
				}
			}
			...
			for (ServiceDetail detail : details) {
				...
				if (!servicePropertyOptionsMap.containsKey(detail.getServiceName()))
					servicePropertyOptionsMap.put(detail.getServiceName(), ServicePropertyHelper.createHelperList(properties));
				else
	...
	public void handleDebugEvents(DebugEvent[] events) {
		for (int i = 0; i < events.length; i++) {
			if (events[i].getSource() instanceof RuntimeProcess) {
				String type = ((RuntimeProcess) events[i].getSource()).getAttribute(StartVBUGSelectionListener.TYPE);
