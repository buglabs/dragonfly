...
import com.buglabs.dragonfly.cdt.ManagedCommandLineInfo;

/**
 * Commandline generator for Angstrom gcc toolchain.
 * 
 * @author kgilmer
 *
 */
public class PreInstalledCommandLineGenerator implements IManagedCommandLineGenerator {
	...
	public IManagedCommandLineInfo generateCommandLineInfo(ITool tool, String commandName, String[] flags, String outputFlag, String outputPrefix, String outputName,
			...
			if (stop > 0 && stop <= commandLinePattern.length())
				try {
					...
					if (varName.compareToIgnoreCase(CMD_LINE_PRM_NAME) == 0)
						...
						sb.append(stringArrayToString(flags));
					...
					} else if (varName.compareToIgnoreCase(INPUTS_PRM_NAME) == 0 && (inputResources != null)) {
						StringBuffer tempBuffer = new StringBuffer(EMPTY);
						...
						sb.append(VAR_FIRST_CHAR + VAR_SECOND_CHAR + varName + VAR_FINAL_CHAR);
				} catch (Exception ex) {
		...
		return new ManagedCommandLineInfo(PreInstalledToolchainSupportValidator.ANGSTROM_DEFAULT_TOOLCHAIN_ROOT + "/arm/bin/" + sb.toString().trim(), commandLinePattern, commandName, stringArrayToString(flags), outputFlag,
				outputPrefix, outputName, stringArrayToString(inputResources));
	}
