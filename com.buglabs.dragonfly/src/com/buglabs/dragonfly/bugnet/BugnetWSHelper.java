...
public class BugnetWSHelper {
	 ...
	 * 
	 * @param username
	 *            String name of owner of program
	 * @return InputStream representing program jar
	 * @throws BugnetException
	 * @throws BugnetAuthenticationException
	 */
	public static InputStream getProgram(String project) throws BugnetAuthenticationException, BugnetException, IOException {
		InputStream isout = null;
		try {
			String urlStr = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_PROGRAM_STRING + "/" + URLEncoder.encode(project, "UTF-8");
	...
	/**
	 * Add a program to BUGnet first checks if it exists via HEAD if it exists,
	 * attempts a PUT to update program if it doesn't, attempts a post
	 * 
	 * @param jarfile
	 * @param name
	 * @throws IOException
	 ...
	 */
	public static String addProgram(File jarfile, String name) throws BugnetAuthenticationException, BugnetException, IOException {
		HTTPRequest request = new HTTPRequest(new BugnetConnectionProvider());
		...
		try {
			if (status == HTTPResponse.HTTP_CODE_NOT_FOUND) {
				response = request.post(urlStr + "?" + BUGNET_TITLE_PARAM_STRING + title, new FileInputStream(jarfile));
	...
	/**
	 * Use this one if you want a token as the proper thing to do is get the
	 * token for the current logged in user, which is persisted in
	 * AuthenticationData
	 * 
	 * @return
	 * @throws BugnetAuthenticationException
	 ...
	 * @throws IOException
	 */
	public static String getToken() throws BugnetAuthenticationException, BugnetException, IOException {
		// get current user's info
		AuthenticationData authentication_data = BugnetStateProvider.getInstance().getAuthenticationData();
	...
	public static boolean verifyCurrentUser() throws IOException {
		String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_USERS_STRING + "/" + BUGNET_VERIFY_STRING;
		HTTPRequest request = new HTTPRequest(new BugnetConnectionProvider());
		...
		if (response_code == HTTPResponse.HTTP_CODE_OK)
			return true;
		else
			return false;
	}
	 ...
	 * @throws IOException
	 */
	public static List getPrograms(String querystring) throws BugnetAuthenticationException, BugnetException, IOException {
		String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_PROGRAM_STRING + "?" + querystring;
	 ...
	 * @throws IOException
	 */
	public static List getProgramsForUser(String user, String querystring) throws BugnetAuthenticationException, BugnetException, IOException {
		List apps = new ArrayList();
		if (!user.trim().equals("")) {
			// /users/' + testface.login + '/programs
			String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_USERS_STRING + "/" + user + "/" + BUGNET_PROGRAM_STRING + "?" + querystring;
	 ...
	 * @throws IOException
	 */
	public static List getUserApps(String user) throws BugnetAuthenticationException, BugnetException, IOException {
	 ...
	 * @throws IOException
	 */
	public static List getUserApps(String user, int count) throws BugnetAuthenticationException, BugnetException, IOException {
		List apps = new ArrayList();
		if (!user.trim().equals("")) {
			// /users/' + testface.login + '/programs
			String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_USERS_STRING + "/" + user + "/" + BUGNET_PROGRAM_STRING + "?" + BUGNET_COUNT_PARAM_STRING + count;
			apps = getProgramList(url);
		}
		return apps;
	}
	 ...
	 * @throws IOException
	 */
	public static List getTopApps(int count) throws BugnetAuthenticationException, BugnetException, IOException {
		// http://ticker/ws/1/program?count=25&sort=rating
		String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_PROGRAM_STRING + "?" + BUGNET_COUNT_PARAM_STRING + count + "&" + BUGNET_SORT_BY_RATING_PARAM_STRING;
		return getProgramList(url);
	}
	 ...
	 * @throws IOException
	 */
	public static List getLatestApps(int count) throws BugnetAuthenticationException, BugnetException, IOException {
		// http://ticker/ws/1/program?count=25
		String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_PROGRAM_STRING + "?" + BUGNET_COUNT_PARAM_STRING + count + "&" + BUGNET_SORT_BY_UPDATED_PARAM_STRING + "&"
				+ BUGNET_UPDATED_SORT_ORDER_PARAM_STRING;
		return getProgramList(url);
	}

	/**
	 * @deprecated - use getPrograms(querystring) and set package names in
	 *             querystring
	 * 
	 *             Given a list of package names returns XML representation of
	 *             the programs in the form of a {@link List} of
	 *             {@link BUGNetProgramReferenceNode}s
	 * 
	 * @param packageList
	 ...
	 * @throws IOException
	 */
	public static List getProgramsForPackages(List packageList) throws BugnetAuthenticationException, BugnetException, IOException {
		String packages = "";
		...
		String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_PROGRAM_STRING + "?" + BUGNET_PACKAGES_PARAMS_STRING + packages;
		return getProgramList(url);
	}
	 ...
	 * @throws BugnetAuthenticationException
	 */
	private static void handleBugnetException(HTTPException e) throws BugnetAuthenticationException, BugnetException {
	...
	/**
	 * 
	 * Private to keep you from using it. Get's called from getToken() Shouldn't
	 * use it because you should only be getting your own token Which means that
	 * the username should be the same as the username/password stored in
	 * AuthenticationData.
	 * 
	 * @param username
	 * @return
	 ...
	 * @throws IOException
	 */
	private static String getToken(String username) throws BugnetAuthenticationException, BugnetException, IOException {
		String url = getBugNetBaseURL() + BUGNET_WS_PATH_STRING + BUGNET_USERS_STRING + "/" + username + "/" + BUGNET_TOKEN_STRING;
		HTTPRequest request = new HTTPRequest(new BugnetConnectionProvider());
		String token = "";
		try {
			HTTPResponse response = request.get(url);
	 ...
	 * @throws IOException
	 */
	private static List getProgramList(String url) throws BugnetAuthenticationException, BugnetException, IOException {
		List apps = new ArrayList();
		try {
			HTTPRequest request = new HTTPRequest(new BugnetConnectionProvider());
			HTTPResponse response = request.get(url);
	...
	/**
	 * helper code copied from old BugnetWSHelper to create List of programs
	 * from xml
	 * 
	 * @param root
	 * @return
	...
	private static List getProgramReferenceNodes(XmlNode root) throws IOException {
		...
		for (Iterator i = prgList.iterator(); i.hasNext();) {
			...
			if (name != null && webid != null && bugNetUrl != null) {
				...
				BUGNetProgramReferenceNode node = new BUGNetProgramReferenceNode(webid, name, username, bugNetUrl, description, imageKey, download_count, rating, apiVersion);
	...
	/**
	 * Helper code from old BugnetWSHelper, modified to NOT use WSHelper for
	 * getImage
	 * 
	 * @param location
	 * @return
